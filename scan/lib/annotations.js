...
const sarifPaths_1 = require("./sarifPaths");
 ...
 * @param originalUriBaseIds URI bases used to resolve the artifact location.
 * @returns GitHub Check annotations are created for each result.
 */
function parseResult(result, rules, originalUriBaseIds) {
    var _a, _b, _c;
    if (!result.locations ||
...
function parseSarif(path) {
    ...
    if ((_b = run.results) === null || _b === void 0 ? void 0 : _b.length) {
        annotations = run.results
            ...
            .map(result => parseResult(result, rules, originalUriBaseIds))
