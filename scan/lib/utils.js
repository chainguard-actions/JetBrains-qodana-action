...
(0, utils_1.setDeprecationWarningCallback)((message) => {
...
function getInputs() {
    ...
    const nativePrefix = (0, qodana_1.getNativeModePrefix)(argList);
    cachedInputs = {
        args: argList,
...
function getPrSha() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const fetchResult = yield gitOutput(['fetch', 'origin', targetBranch, sourceBranch], { ignoreReturnCode: true });
        if (fetchResult.exitCode === 0) {
            const mergeBase = yield gitOutput(['merge-base', `origin/${targetBranch}`, `origin/${sourceBranch}`], { ignoreReturnCode: true });
            if (mergeBase.exitCode === 0) {
                return mergeBase.stdout.trim();
            }
        }
        // Step 2: Fall back to webhook payload SHAs
        core.warning('Unable to compute merge-base using git. ' +
            'Please specify fetch-depth: 0 in your checkout step. ' +
            'Falling back to the GitHub webhook payload.');
        const webhookMergeBase = yield gitOutput(['merge-base', pr.base.sha, pr.head.sha], { ignoreReturnCode: true });
        if (webhookMergeBase.exitCode === 0) {
            return webhookMergeBase.stdout.trim();
        }
        // Step 3: Nothing worked — disable PR mode
        core.warning('Unable to determine the base commit for PR analysis. ' +
...
function warnIfMergeCommitCheckout() {
    return __awaiter(this, void 0, void 0, function* () {
        const { stdout } = yield gitOutput(utils_1.MERGE_COMMIT_PARENTS_ARGS, {
            ignoreReturnCode: true
        });
        if ((0, utils_1.isMergeCommit)(stdout)) {
            core.warning(`${utils_1.MERGE_COMMIT_PR_WARNING} ` +
...
function verifyInstalledCli() {
    return __awaiter(this, void 0, void 0, function* () {
        const result = yield exec.getExecOutput(qodana_1.EXECUTABLE, ['--version'], {
            ignoreReturnCode: true,
            silent: true
        });
...
function installCli(nightlyVersion) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const nightlyTag = yield (0, qodana_1.getNightlyTag)(nightlyVersion);
        const temp = yield tc.downloadTool((0, qodana_1.getQodanaUrl)(arch, platform, nightlyTag));
        if (!nightlyVersion) {
        ...
        core.addPath(yield tc.cacheDir(extractRoot, qodana_1.EXECUTABLE, nightlyVersion ? `nightly-${nightlyVersion}` : qodana_1.VERSION));
    });
}
function prepareAgent(args_1) {
    return __awaiter(this, arguments, void 0, function* (args, nightlyVersion = '', useInstalledCli = false) {
        if (useInstalledCli) {
            yield verifyInstalledCli();
        }
        else {
            yield installCli(nightlyVersion);
        }
        if (!(0, qodana_1.isNativeMode)(args) && !(0, qodana_1.isPullSkipped)(args)) {
 ...
 */
function putReaction(githubToken, newReaction, oldReaction) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a;
        const pr = github.context.payload.pull_request;
        if (!pr) {
        ...
        let ownLogin;
        try {
            const { data: created } = yield client.rest.reactions.createForIssue(Object.assign(Object.assign({}, github.context.repo), { issue_number, content: newReaction }));
            ownLogin = (_a = created.user) === null || _a === void 0 ? void 0 : _a.login;
        }
        catch (error) {
        ...
        if (oldReaction !== '' && oldReaction !== newReaction && ownLogin) {
            try {
                const reactions = yield client.paginate(client.rest.reactions.listForIssue, Object.assign(Object.assign({}, github.context.repo), { issue_number, per_page: 100 }));
                for (const previousReaction of reactions.filter(r => { var _a; return r.content === oldReaction && ((_a = r.user) === null || _a === void 0 ? void 0 : _a.login) === ownLogin; })) {
                    try {
                        ...
                        core.debug(`Failed to delete reaction ${previousReaction.id} – ${error.message}`);
                    }
                }
            }
            catch (error) {
                core.warning(`Failed to list the initial reactions – ${error.message}`);
            }
        }
    });
}
